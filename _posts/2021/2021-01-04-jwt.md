...
* JWT在线验证工具；
* JWT的格式；
* JWT的核心要素: Header, Payload, Signature；
* Base64Url (vs Base64)；
* 使用JWT进行会话管理: 主题和期限
* HS256签名 – 它是如何工作的？
* 数字签名；
* 哈希函数和SHA-256；
* RS256 JWT签名 – 谈谈公钥加密；
* RS256 vs HS256 签名 – 哪种方式更好？
* JWKS (JSON Web Key Set) 端点(Endpoints)；
* 如何实现JWT签名的周期性键旋转(Periodic Key Rotation)；
* JWT在企业应用中的使用；
* 归纳和结论；
...
让我们来看看JWT的实际例子，这是从<http://jwt.io>的JWT校验工具得到的：
...
如果你还想确认这些信息是否真的存在，可以拷贝JWT串到<http://jwt.io>的在线校验工具校验一下即可。
...
我们看下<http://jwt.io>上的例子，一个使用RS256加签的JWT。
...
* [1] [想全面理解JWT？一文足矣！](https://zhuanlan.zhihu.com/p/70275218)
